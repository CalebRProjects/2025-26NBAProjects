...
```{r net_impact_vs_minutes, fig.cap="Net impact vs minutes played"}
queta_games <- tibble(
  game = 1:33,
  minutes = c(
...
ggplot(
  pt_off,
  aes(x = ppp, y = reorder(playtype, ppp))
) +
  geom_point(
    aes(size = poss),
    color = CELTICS_GREEN,
  ...
  labs(
    ...
    subtitle = glue(
      "Size: Volume. Dashed-line: Weighted avg PPP ({round(w_off_ppp, 3)})."
    ),
    x = "PPP",
    y = NULL,
