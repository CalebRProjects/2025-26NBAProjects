...
params:
  ...
  min_minutes: 200
  cache_dir: "data/processed/shots"
header-includes:
  - \usepackage{float}
...
SEASON_PRIOR <- params$season_prior
PLAYER_NAME <- params$player_name
MIN_MINUTES <- params$min_minutes
SHOTS_DIR <- params$cache_dir
...
GRID <- "#dedad3"
KINGS_PURPLE <- "#5A2D81"
CAPTION_LINE <- "Data: NBA API via hoopR | Viz: @Rambzee_"
...
theme_caleb_rmd <- function() {
  theme_minimal(base_family = "poppins") +
    theme(
      ...
      plot.title = element_text(size = 20, face = "bold", hjust = 0.5, color = TEXT_DARK),
      plot.subtitle = element_text(size = 9.5, hjust = 0.5, color = TEXT_MUTED, margin = margin(b = 6)),
...
pct_rank <- function(x, value) mean(x <= value, na.rm = TRUE) * 100
...
plot_usage_vs_eff_base(SEASON_PRIOR, MIN_MINUTES)
plot_usage_vs_eff_base(SEASON_CURRENT, MIN_MINUTES)
```
...
- His field-goal efficiency remains above league average in 2024-25 and remains just below league average through this season.
...
keon_id_2425 <- get_player_id(SEASON_PRIOR, PLAYER_NAME)
keon_id_2526 <- get_player_id(SEASON_CURRENT, PLAYER_NAME)

shots_2425 <- get_cached_shots(keon_id_2425, SEASON_PRIOR)
shots_2526 <- get_cached_shots(keon_id_2526, SEASON_CURRENT)

plot_binned_shotchart(shots_2425, paste0(PLAYER_NAME, " shot chart (", SEASON_PRIOR, ")"))
plot_binned_shotchart(shots_2526, paste0(PLAYER_NAME, " shot chart (", SEASON_CURRENT, ")"))
```

His shot charts show a tightly constrained shot diet concentrated on capitalizing on advantages created by him and his teammates:
...
plots_2425 <- make_keon_def_plots(SEASON_PRIOR, min_minutes = MIN_MINUTES)
plots_2526 <- make_keon_def_plots(SEASON_CURRENT, min_minutes = MIN_MINUTES)
