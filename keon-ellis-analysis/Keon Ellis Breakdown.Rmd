...
```{r ctg data, echo = FALSE, message=FALSE, warning=FALSE}
ctg_onoff = tibble(
...
ctg_onoff %>%
  transmute(
    ...
    ) %>% kable(caption = "Cleaning the Glass – On/Off Differentials (On minus Off)")
...
```{r databallr data, echo = FALSE, message=FALSE, warning=FALSE}
...
databallr %>%
  transmute(
    ...
    `STL%` = round(stl_pct, 1),
    `BLK%` = round(blk_pct, 1),
    `STOP%` = round(stop_pct, 1)) %>% kable(caption = "Databallr – Impact and Event Creation")
```
...
plot_usage_vs_eff_base = function(season_used = "2024-25",
                                  min_minutes = 200,
                                  keon_name = "Keon Ellis",
                                  kings_purple = "#5A2D81") {

  df = hoopR::nba_leaguedashplayerstats(
    season = season_used,
  ...
  keon = df %>% filter(PLAYER_NAME == keon_name)
  if (nrow(keon) == 0) stop("Keon not found in Base table. Check spelling or season.")

  keon = keon %>% slice_tail(n = 1)

  ggplot(df, aes(x = fga_per_36, y = FG_PCT)) +
    ...
    labs(
      ...
      subtitle = paste0("Regular Season ", season_used, " | Min ", min_minutes, "+ minutes"),
...
add_standard_shot_zones = function(shots) {
  ...
  shots %>%
    mutate(
      ...
      rim_5ft = 60,     # 5 ft = 60 in
      paint_10ft = 120, # 10 ft = 120 in
...
get_cached_shots = function(player_id, season) {
  ...
  shots = sc$Shot_Chart_Detail %>%
    mutate(
      ...
      game_date = ifelse(is.na(game_date), suppressWarnings(lubridate::ymd(as.character(GAME_DATE))), game_date),
...
get_player_id = function(season, player_name = "Keon Ellis") {
  ...
  pid = lg %>% filter(PLAYER_NAME == player_name) %>% slice_tail(n = 1) %>% pull(PLAYER_ID) %>% .[1]
  if (length(pid) == 0 || is.na(pid)) stop("PLAYER_ID not found. Check season string or player spelling.")
  pid
}
...
```{r efficiency by context, echo = FALSE, message=FALSE, warning=FALSE}
ctx = tibble::tibble(
...
ctx %>%
  ...
  knitr::kable(caption = "Efficiency by Context (Databallr)")
```
...
get_league_def = function(season = "2024-25") {
  hoopR::nba_leaguedashplayerstats(
    ...
    mutate(
      ...
      PCT_STL = suppressWarnings(
        as.numeric(gsub("[^0-9\\.]", "", PCT_STL))
      )
    )
}
...
ordinal = function(x) {
  ...
  last = x %% 10
  suf = ifelse(last == 1, "st", ifelse(last == 2, "nd", ifelse(last == 3, "rd", "th")))
  paste0(x, suf)
}

plot_keon_density = function(df, metric_col, metric_label,
                             season_label,
                             keon_name = "Keon Ellis",
                             min_minutes = 200,
                             kings_purple = "#5A2D81") {
  ...
  keon = df2 %>% filter(PLAYER_NAME == keon_name) %>% slice_tail(n = 1)

  if (nrow(keon) == 0) stop("Keon not found in this season table. Check spelling or season.")

  keon_val = keon[[metric_col]][1]
  lg_avg = mean(df2[[metric_col]], na.rm = TRUE)
  pctl = pct_rank(df2[[metric_col]], keon_val)

  # nice annotation text
  ann_txt = paste0(
    keon_name, "\n",
    round(keon_val, 2), " ", metric_label, "\n",
    round(pctl, 1), "th percentile"
  )
  ...
  subtitle_txt = paste0(
    ...
    " | Keon: ", round(keon_val, 2), " (", round(pctl, 1), "th pct)"
  )

  ggplot(df2, aes(x = .data[[metric_col]])) +
    geom_density(fill = "grey85", alpha = 1) +

    # league average (dashed dark gray)
    geom_vline(xintercept = lg_avg, linetype = "dashed", linewidth = 0.9, color = "#4b4b4b") +

    # Keon (Kings purple)
    geom_vline(xintercept = keon_val, linewidth = 1.2, color = kings_purple) +

    # label box anchored near top
    annotate(
      "label",
      x = keon_val,
      y = Inf,
    ...
    # keep caption safely bottom-left, never overlapping plot
    theme(plot.caption.position = "plot")
}
...
make_def_rates = function(df) 
  ...
  df %>%
    mutate(
      stl_per_36 = ifelse(MIN > 0, STL / (MIN / 36), NA_real_),
      stl_pct = PCT_STL
    )
  }
...
make_keon_def_plots = function(season_used = "2024-25", min_minutes = 200) {
  league_def = get_league_def(season = season_used) %>% make_def_rates()
...
League-wide distributions show Ellis in the 96th percentile in both steals per 36 minutes and steal percentage. Importantly, these events do not come at the expense of team structure. His STOP% climbed from 4.3% in 2024–25 to 5.3% in 2025–26, while Defensive DPM and RAPM remain consistently positive. He generates stops at an elite rate for a guard, even in lackluster defensive circumstances.
