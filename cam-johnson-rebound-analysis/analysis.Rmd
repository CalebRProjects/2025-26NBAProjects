...
caption_games <- "Data: NBA game logs via hoopR | Viz: @Rambzee_"
caption_shots <- "Data: NBA ShotChartDetail via hoopR | Viz: @Rambzee_"
...
style_latex_table <- function(table_obj, scale_down = TRUE) {
  latex_options <- if (scale_down) c("hold_position", "scale_down") else "hold_position"
...
get_player_gamelogs <- function(season) {
  # NBA.com/hoopR response schemas can change. This keeps the original endpoint logic intact.
...
get_player_shots <- function(player_id, season) {
  ...
  if (!"DRIBBLES" %in% names(raw)) {
    raw$DRIBBLES <- NA
  }

  if (!"SHOT_DISTANCE" %in% names(raw)) {
    raw$SHOT_DISTANCE <- NA
  }
...
```{r data-loading}
games <- get_player_gamelogs(season_used)

cam_games <- games %>%
  filter(player_name == target_player) %>%
  arrange(game_date)

if (nrow(cam_games) < rolling_k + 2) {
...
shots <- get_player_shots(cam_id, season_used)
```
...
plot_roll <- cam_games_clean %>%
  mutate(
    reb36_roll = roll_mean_partial(reb36, rolling_k),
    ts_roll = roll_mean_partial(ts_use, rolling_k),
    three_roll = roll_mean_partial(three_share, rolling_k)
  ) %>%
...
Where the difference emerges is in execution. Improved efficiency, paired with a meaningful increase in minutes played, translated into stronger counting production. The combination of role clarity, lineup fit, and increased court time produced the type of impact Denver envisioned when acquiring Johnson.

## Shooting Profile

```{r three-zone-profile, fig.cap="Where the 3s are coming from."}
...
The shot diet breakdown shows only marginal structural change. Johnson is taking slightly more threes overall, but the location distribution closely mirrors his early-season profile. The difference lies in outcomes.
