...
jjj_stocks_gt_reb_all <- jjj_games_all %>%
  mutate(
    ...
    MIN       = round(readr::parse_number(MIN), 1),
    REB       = as.numeric(REB),
    STL       = as.numeric(STL),
    BLK       = as.numeric(BLK),
    stocks    = STL + BLK
...
season_summary_with_total <- bind_rows(
  ...
  season_summary %>%
    summarise(
      SEASON_YEAR = "Career",
      games_total = sum(games_total, na.rm = TRUE),
      reg_season  = sum(reg_season,  na.rm = TRUE),
      playoffs    = sum(playoffs,    na.rm = TRUE)
    )
)
...
game_list_gt <- jjj_stocks_gt_reb_all %>%
  ...
  fmt_number(columns = c(MIN, REB, STL, BLK, stocks), decimals = 0) %>%
  cols_label(
    game_date = "Date",
    SEASON_YEAR = "Season",
    MATCHUP = "Matchup",
...
write.csv(jjj_stocks_gt_reb_all,
