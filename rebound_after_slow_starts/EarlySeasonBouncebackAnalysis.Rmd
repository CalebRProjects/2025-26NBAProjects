...
{
  gamelog <- hoopR::nba_leaguegamelog(
    season = season,
...
games <- get_player_gamelogs(season = "2025-26")
```
...
shots_cache_dir <- "rebound_after_slow_starts/data/processed/shots"
dir.create(shots_cache_dir, recursive = TRUE, showWarnings = FALSE)

get_cached_shots <- function(player_id, season = "2025-26")
{
  path <- file.path(shots_cache_dir, paste0("shots_", player_id, ".csv"))
...
add_zone <- function(df)
{
  df %>%
    mutate(
      ...
      zone = factor(
        zone,
        levels = c("Rim", "Paint (non-rim)", "Midrange", "Corner 3", "Above-the-break 3", "Other")
      )
    )
}
...
plot_df <- lapply(params$players, function(p) {
  ...
  team <- games %>%
    filter(player_name == p) %>%
    slice_tail(n = 1) %>%
    pull(team) %>%
    .[1]

  early <- summarize_window(sp$early) %>%
    mutate(player = p, window = "Early", team = team)

  recent <- summarize_window(sp$recent) %>%
    mutate(player = p, window = "Recent", team = team)
...
for (player in params$players) {
  ...
  delta_tbl <- deltas_row %>%
    transmute(
    `PPG d`        = fmt_delta(ppg_delta, 1),
    `AST/G d`      = fmt_delta(ast_pg_delta, 1),
    `REB/G d`      = fmt_delta(reb_pg_delta, 1),
    `TS% d`        = fmt_pct_delta(ts_delta, 0.1),
    `MPG d`        = fmt_delta(mpg_delta, 1),
    `FGA/G d`      = fmt_delta(fga_pg_delta, 1),
    `3PA Share d`  = fmt_pct_delta(three_rate_delta, 1),
    `FT Rate d`    = fmt_pct_delta(ft_rate_delta, 1),
    `TOV/G d`      = fmt_delta(tov_pg_delta, 2)
    )
  ...
  shots <- get_cached_shots(player_id = player_id, season = "2025-26")
  shots_split <- build_player_shots_split(games_df = games, shots_df = shots, player_name = player)
  ...
  # Paragraph
  cat(write_player_paragraph(player, halves_tbl, deltas_row, zone_deltas), "\n\n")
